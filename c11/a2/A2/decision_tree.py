...
class DecisionTree:
    ...
    def _entropy(self, y):
        ...
        with np.errstate(divide='ignore', invalid='ignore'):
            total = np.sum(counts)
            probs = np.where(total == 0, 0, counts / total).astype(float)
            logProbs = np.log2(np.where(probs == 0, 1, 1/probs).astype(float))
    ...
    def _find_split(self, X, y, H_data):
        ...
        split_value = maximum_information_gain = 0
        ...
        for ii in range(unique_values.shape[0] - 1):
            ...
            leftProb = (X[:, split_dim] < current_split_value).sum()/N
            rightProb = (X[:, split_dim] > current_split_value).sum()/N
            current_information_gain = H_data - leftProb*H_left - rightProb*H_right
