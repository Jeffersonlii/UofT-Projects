...
TEST(extract_min_ordered) {
  ...
  for (int i = 0; i < 10; i++)
    ...
    if (heapExtractMin(heap, &pri) != i || pri != i)
      TEST_FAIL("ExtractMin did not return correct values, expected %i, %i, got %i, %f\n", i, i, heapExtractMin(heap, &pri), pri);
...
TEST(extract_min_random) {
  ...
  for (int i = 0; i < 15; i++){
    ...
    // for (int j = 0; j < heap->numItems; j++)
    // {
    //   printf("i = %i, %f ", j, heap->arr[j].priority);
    //   printf("\n");
    // }

    int sii = heapExtractMin(heap, &pri);

    // printf("cor  %i, %f \n", si[i], p[si[i]]);
    // printf("got  %i, %f \n", sii, pri);
    // printf("same? %s \n\n", pri == p[si[i]] ? "t" : "f");

    if (sii != si[i] || pri != p[si[i]])
