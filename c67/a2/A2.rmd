...
Since we know 
$$E(Y_0) = \hat{Y_0} \sim N(\mu, \sigma^2),\text{where } \mu =  \hat{\beta_0} + \hat{\beta_1} \cdot X_0 = `r Bhat0` + `r Bhat1` \cdot `r X0` = `r Bhat0 + Bhat1 * X0`, \quad \sigma^2 = 25 $$
```{r}
lower = qnorm(0.025, Bhat0+Bhat1 * X0, 5)
upper = qnorm(0.975, Bhat0+Bhat1 * X0, 5)
```
The 95% confidence interval for E(Y0) when X0 = 10 is given by $$[`r lower`, `r upper`]$$
